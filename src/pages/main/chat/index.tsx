...
const ChatGPT: React.FC = () => {
  ...
  const [chatHistory, setChatHistory] = useState<Chat[]>(
    JSON.parse(window.localStorage.getItem('chat-history') as any) || [],
  );
  ...
  const handleUserInput = async () => {
    ...
    setChatHistory((prevChatHistory) =>
      prevChatHistory.concat(newUserMessage as any),
    );
    toBottom();
    setLoading(true);
    try {
      const chatCompletionContent = await getChatContent(
      ...
      setChatHistory((prevChatHistory) =>
        prevChatHistory.concat(assistantMessage as any),
      );
      setLoading(false);
    } catch (e: any) {
      message.error(e);
  ...
  return (
    <div className="chat-container">
        ...
        onOk={() => {
          ...
          if (!window.localStorage.getItem('chat-history-keep')) {
            window.localStorage.setItem(
              'chat-history-keep',
              JSON.stringify([{ name: inputValue, value: chatHistory }]),
            );
          } else {
            const historyArray = JSON.parse(
              window.localStorage.getItem('chat-history-keep') as string,
            ) as ChatHistoryKeepType[];
      ...
      <Drawer
        ...
        open={drawerOpen}
      >
        {!localStorage.getItem('chat-history-keep') && (
          <Empty
            image={Empty.PRESENTED_IMAGE_SIMPLE}
            description="您还未保存任何聊天记录"
          />
        )}
        {!!localStorage.getItem('chat-history-keep') && (
          <ul>
            {(
              JSON.parse(
                localStorage.getItem('chat-history-keep') as string,
              ) as ChatHistoryKeepType[]
            ).map((item, index) => (
              <>
